...
SELECT COUNT(DISTINCT country) AS paisos_compradors
FROM company AS c
JOIN transaction AS t ON c.id = t.company_id
WHERE t.declined=0;
...
SELECT DATE_FORMAT(timestamp, '%m-%d-%y') AS date, SUM(amount) AS total_day_sales
FROM transaction
WHERE declined=0
GROUP BY date
...
SELECT country, AVG(amount) AS sales_avg
FROM company AS c
JOIN transaction AS t ON c.id = t.company_id
GROUP BY country
...
SELECT company_name, phone, country, DATE_FORMAT(timestamp, '%m-%d-%y') AS date, amount
FROM company AS c
JOIN transaction AS t ON c.id = t.company_id
